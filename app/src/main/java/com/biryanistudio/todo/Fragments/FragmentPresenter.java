...
class FragmentPresenter {
	...
	void clearPendingTasks() {
		DbTransactions.updateAllPendingTasksAsCompleted(fragment.getContext());
		swapCursorOnAdapter(getAppropriateCursor());
	}

	void clearCompletedTasks() {
		DbTransactions.deleteAllCompletedTasks(fragment.getContext());
		swapCursorOnAdapter(getAppropriateCursor());

	}

	private void swapCursorOnAdapter(Cursor cursor) {
		if ( cursor != null ) {
			if ( cursor.getCount() != 0 ) {
